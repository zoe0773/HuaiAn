...
class DeyuPage(Base):
    ...
    zb_name = (By.CSS_SELECTOR, "div.item-box:nth-child(2)>div.input>input.name")  # 指标名称输入框
    ...
    def dy_menu(self, menu_name):
        ...
        #通过menu_name选择要创建指标或分类的德育分类（班级德育、学生德育、教师德育）
        ele = self.driver.find_element_by_xpath("//div[contains(@class, 'parent-node')]/span"     
                                          "/label[text()='%s']/parent::span/fllowing-sibling::div"
                                          "/span[@data-type='node-add']" % menu_name)  #定位的是添加按钮
        ele.click()

        '''通过class 包括xxx的div元素查找span下的label标签，
        基于label标签查找到父元素span,于span的位置查找之后的兄弟元素div,
        并基于此位置查找到@d-t=node-add 属性的span元素'''
